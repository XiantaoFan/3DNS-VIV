...
subroutine parameter(input_i3d)
  ...
  use forces, only : iforces, xld, xrd, yld, yud!, zld, zrd
  ...
  NAMELIST /ibmstuff/ cex,cey,cez,ra,nobjmax,nraf,nvol,iforces, npif, izap, ianal, imove, thickness, chord, omega ,ubcz,rads, c_air, idyn, ax, ay, az, mass, fn_x, fn_y, epsion
  ...
  if (iibm.ne.0) then
   allocate(mass(ncyl), epsion(ncyl), fn_y(ncyl), fn_x(ncyl),ra(ncyl),idyn(ncyl),imove(ncyl), cex(ncyl), cey(ncyl))
   allocate(ax(ncyl),ay(ncyl),az(ncyl))
   allocate(lift_tr(ncyl),drag_tr(ncyl))
   allocate (n_body_centroid(ncyl,2), n_body_vel(ncyl,2))
