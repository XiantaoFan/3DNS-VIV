...
module param
  ...
  integer :: itime0
  integer :: iscalar,nxboite,istat,iread,iadvance_time,irotation,iibm, ncyl
...
module ibm_param
  use decomp_2d, only : mytype
  real(mytype) :: ubcx,ubcy,ubcz,rads, c_air,cez !=====added by Xiantao Fan, dynamic parameter
  real(mytype), allocatable, dimension(:)  :: cex,cey, ra, ax, ay, az, mass, fn_x, fn_y, epsion  !=====added by Xiantao Fan, dynamic parameter
  real(mytype) :: chord,thickness,omega
  real(mytype), allocatable, dimension(:) :: lift_tr,drag_tr
  real(mytype), allocatable, dimension(:,:):: n_body_centroid, n_body_vel
  
  integer :: inana ! Analytical BC as Input
  integer :: nvol
  integer, allocatable, dimension(:)  :: imove
  integer, allocatable, dimension(:)  :: idyn  !====moving kind
