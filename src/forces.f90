...
contains
  ...
  subroutine force(ux1,uy1,ep1)
    ...
    real(mytype) ,allocatable:: yLift_mean,xDrag_mean 
...
do iv=1,nvol
    ...
    if(imove(iv).eq.1) then
        if (idyn(iv).eq.1.) then
         ubcx=ax(iv)
         ubcy=ay(iv)
    elseif (idyn(iv).eq.2.) then
         ubcx = 2* pi * fn_x(iv) * ax(iv) * cos (2* pi * fn_x(iv) *(t-ifirst*dt))
         ubcy = 2* pi * fn_y(iv) * ay(iv) * cos (2* pi * fn_y(iv) *(t-ifirst*dt))
    elseif (idyn(iv).eq.3.) then 
         ubcx = 0
         ubcy = n_body_vel(iv,2)       
    elseif (idyn(iv).eq.4.) then 
         ubcx = n_body_vel(iv,1)
         ubcy = n_body_vel(iv,2)
    endif
   endif 
...
!!$!(jcvlw) A____________D      
       ...
       lift_tr(iv)=yLift_mean
       drag_tr(iv)=xDrag_mean
       ...
       !if (nrank .eq. 0) then
        !  write(38+iv,*) t,xDrag_mean,yLift_mean
         ! call flush(38+iv)
       !endif
    if (nrank .eq. 0) then
       write(filelist,"(i2)") iv
       open(38, file="Forces-"//trim(AdjustL(filelist))//".dat", position='Append',mode ='write') 
       write(38,*) t,drag_tr(iv),lift_tr(iv)
       call flush(38)
    endif
...
if (idyn (iv).eq.3) then
  if (nrank .eq. 0) then
    write(filelist,"(i2)") iv
    open(48, file="1-DOFResponses-"//trim(AdjustL(filelist))//".dat", position='Append',mode ='write') 
    write(48,*) t,n_body_centroid (iv,2), n_body_vel(iv,2)
    call flush(48)
  endif
endif
...
if (idyn(iv).eq.4) then
  if (nrank .eq. 0) then
    write(filelist,"(i2)") iv
    open(48, file="2-DOFResponsesx-"//trim(AdjustL(filelist))//".dat", position='Append',mode ='write') 
    write(48,*) t,n_body_centroid (iv,1), n_body_vel(iv,1)
    call flush(48)
    open(58, file="2-DOFResponsesy-"//trim(AdjustL(filelist))//".dat",position='Append', mode ='write') 
    write(58,*) t,n_body_centroid (iv,2), n_body_vel(iv,2)
    call flush(58)
  endif
endif
!===================write the dynamic responses 
