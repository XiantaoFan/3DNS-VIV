...
contains
  ...
  subroutine lagpolx(u)
    ...
    integer                                            :: i,j,k
    real(mytype)                                       :: x,y,z
    integer                                            :: ix              !=   position du point "zapp¨¦"
    integer                                            :: ipif,ipol,nxpif
    integer                                            :: ipoli,ipolf     !=   positions Initiales et Finales du POLyn?me consid¨¦r¨¦
    real(mytype)                                       :: xpol,ypol,dypol   !|variables concernant les polyn?mes
    real(mytype),dimension(10)                         :: xa,ya           !|de   Lagrange. A mettre imp¨¦rativement en
    integer                                            :: ia,na             !|double pr¨¦cision
    !
    do k=1,xsize(3)
       do j=1,xsize(2)
          if(nobjx(j,k).ne.0)then
             ...
             do i=1,nobjx(j,k)          !boucle sur le nombre d'objets par (j,k)
                ...
                xa(ia)=xi(i,j,k)
                ya(ia)=zero
                if(xi(i,j,k) > zero)then!objet immerg¨¦
                   ix=xi(i,j,k)/dx+1
                   ipoli=ix+1
                ...
                xa(ia)=xf(i,j,k)
                ya(ia)=zero
                if(xf(i,j,k) < xlx)then!objet immerg¨¦
                   ix=(xf(i,j,k)+dx)/dx+1
                   ipolf=ix-1
  ...
  subroutine lagpoly(u)
    ...
    integer                                            :: jpoli,jpolf     !=   positions Initiales et Finales du POLyn?me consid¨¦r¨¦
    real(mytype)                                       :: xpol,ypol,dypol   !|variables concernant les polyn?mes
    real(mytype),dimension(10)                         :: xa,ya           !|de   Lagrange. A mettre imp¨¦rativement en
    integer                                            :: ia,na             !|double pr¨¦cision
    !
       ...
       do i=1,ysize(1)
          if(nobjy(i,k).ne.0)then
             ...
             do j=1,nobjy(i,k)          !boucle sur le nombre d'objets par (j,k)
                ...
                xa(ia)=yi(j,i,k)
                ya(ia)=zero
                if(yi(j,i,k) > zero)then!objet immerg¨¦
                ...
                xa(ia)=yf(j,i,k)
                ya(ia)=zero
                if(yf(j,i,k) < yly)then!objet immerg¨¦
  ...
  subroutine lagpolz(u)
    ...
    integer                                            :: kpoli,kpolf     !=   positions Initiales et Finales du POLyn?me consid¨¦r¨¦
    real(mytype)                                       :: xpol,ypol,dypol   !|variables concernant les polyn?mes
    real(mytype),dimension(10)                         :: xa,ya           !|de   Lagrange. A mettre im¨¦rativement en
    integer                                            :: ia,na             !|double pr¨¦cision
    !
    do j=1,zsize(2)
       do i=1,zsize(1)
          if(nobjz(i,j).ne.0)then
             ...
             do k=1,nobjz(i,j)          !boucle sur le nombre d'objets par couple   (i,j)
                ...
                xa(ia)=zi(k,i,j)
                ya(ia)=zero
                if(zi(k,i,j) > zero)then!objet immerg¨¦
                   kz=zi(k,i,j)/dz+1
                   kpoli=kz+1
                ...
                xa(ia)=zf(k,i,j)
                ya(ia)=zero
                if(zf(k,i,j) < zlz)then!objet immerg¨¦
                   kz=(zf(k,i,j)+dz)/dz+1
                   kpolf=kz-1
...
subroutine ana_y_cyl(i,y_pos,ana_res)
  ...
  !
  !=============added by xiantaofan for dynamic computation
 ...
 do p= 1, nvol
  ...
  if (y_pos.gt.ceyy) then     ! Impose analytical BC
      ana_res=ceyy + sqrt(ra(p)**2.0-((i+ystart(1)-1-1)*dx-cexx)**2.0)
  else
      ana_res=ceyy - sqrt(ra(p)**2.0-((i+ystart(1)-1-1)*dx-cexx)**2.0)
  endif     
  !
...
if (imove(p).eq.1) then
  ...
  if (x_pos.gt.cexx) then     ! Impose analytical BC
      ana_res = cexx + sqrt(ra(p)**2.0-(yp(j+xstart(2)-1)-ceyy)**2.0)
  else
      ana_res = cexx - sqrt(ra(p)**2.0-(yp(j+xstart(2)-1)-ceyy)**2.0)
  endif     
  !
